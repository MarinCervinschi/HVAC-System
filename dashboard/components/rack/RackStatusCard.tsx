            ...
            <CardContent>
                <div className="grid gap-4 md:grid-cols-3">
                    <div className="border p-6 rounded-lg space-y-8">
                        <div className="flex flex-row justify-between">
                            <div className="text-sm font-bold text-muted-foreground">Total Devices</div>
                    ...
                    <div className="border p-6 rounded-lg space-y-8">
                        <div className="flex flex-row justify-between">
                            <div className="text-sm font-bold text-muted-foreground">Active Devices</div>
                            <Wifi className="h-4 w-4 text-green-600" />
                        </div>
                        <div className="text-2xl font-bold text-green-600">
                            {rackActive ? rackInfo.smart_objects.reduce((count, smartObject) => 
                                count + (smartObject.actuators?.filter(actuator => actuator.status === "ON").length || 0) + (smartObject.sensors?.length || 0), 0
                            ) : 0}
                        </div>
                    </div>
                    <div className="border p-6 rounded-lg space-y-8">
                        <div className="flex flex-row justify-between">
                            <div className="text-sm font-bold text-muted-foreground">Inactive Devices</div>
                            <WifiOff className="h-4 w-4 text-red-600" />
                        </div>
                        <div className="text-2xl font-bold text-red-600">
                            {rackActive
                                ? rackInfo.smart_objects.reduce((count, smartObject) => 
                                    count + (smartObject.actuators?.filter(actuator => actuator.status === "OFF").length || 0), 0
                                )
                                : rackInfo.smart_objects.reduce((count, smartObject) => 
                                    count + (smartObject.actuators?.length || 0) + (smartObject.sensors?.length || 0), 0
                                )}
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                    <div className="space-y-1">
                        <p className="text-sm font-medium">Rack Status</p>
                        <p className="text-sm text-muted-foreground">
                            {rackActive ? "Rack operational" : "Rack powered off"}
                        </p>
                    </div>
