...
import { Activity, AlertTriangle, Edit, Plus, Settings, Trash2, Zap } from "lucide-react"
import { useState, useEffect } from "react"
import { ACTION_TYPE_LABELS, OPERATOR_LABELS, Policy, PolicyAction, PolicyCondition, SENSOR_TYPE_LABELS } from "@/types/policy"
...
export function PolicyDialog({ smartObjects, roomId }: PolicyDialogProps) {
    ...
    const [editingPolicy, setEditingPolicy] = useState<{policy: Policy, smartObject: SmartObject} | null>(null)
    ...
    const [newPolicySmartObjectId, setNewPolicySmartObjectId] = useState(smartObjects[0]?.id || "")

    // policiesBySmartObject: { [smartObjectId]: Policy[] }
    const [policiesBySmartObject, setPoliciesBySmartObject] = useState<Record<string, Policy[]>>({})
        ...
        const loadAllPolicies = async () => {
            ...
            await Promise.all(
                smartObjects.map(async (so) => {
                    try {
                        const response = await fetch(`/api/room/${roomId}/rack/${so.rack_id || 'default'}/device/${so.id}/policy`)
                        if (response.ok) {
    ...
    const getActionText = (action: PolicyAction) => {
        const commands = Object.entries(action.command.value)
            .map(
                ([key, value]) =>
                    `${ACTION_TYPE_LABELS[key as keyof typeof ACTION_TYPE_LABELS] || key}: ${value}`
            )
            .join("\n")
    ...
    const handleUpdatePolicy = async () => {
        ...
        try {
            const response = await fetch(`/api/room/${roomId}/rack/${smartObject.rack_id}/device/${smartObject.id}/policy/${policy.id}`, {
                method: 'PUT',
            ...
            if (response.ok) {
                ...
                setPoliciesBySmartObject(prev => ({
                    ...prev,
                    [smartObject.id]: prev[smartObject.id].map(p => p.id === policy.id ? updatedPolicy : p)
                }))
    ...
    const handleDeletePolicy = async (policyId: string, smartObject: SmartObject) => {
        if (!confirm('Sei sicuro di voler eliminare questa policy?')) return
        try {
            const response = await fetch(`/api/room/${roomId}/rack/${smartObject.rack_id}/device/${smartObject.id}/policy/${policyId}`, {
                method: 'DELETE',
            })
            if (response.ok) {
                setPoliciesBySmartObject(prev => ({
                    ...prev,
                    [smartObject.id]: prev[smartObject.id].filter(p => p.id !== policyId)
                }))
            }
        } catch {}
    }
    ...
    const handleCreatePolicy = async () => {
        const smartObject = smartObjects.find(so => so.id === newPolicySmartObjectId)
        if (!smartObject) return
        try {
            const response = await fetch(`/api/room/${roomId}/rack/${smartObject.rack_id}/device/${smartObject.id}/policy`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    const updateNewPolicyCondition = (field: keyof PolicyCondition, value: any) => {
        setNewPolicy((prev) => ({
            ...prev,
        ...
        <Dialog>
            ...
            <DialogContent className="min-w-[50%] max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    ...
                    <DialogDescription>Configura thresholds per tutti i sensori</DialogDescription>
                </DialogHeader>
                <div className="grid gap-4 py-4">
                    <div className="flex items-center justify-between">
                        <h3 className="text-lg font-medium">Policy per tutti gli Smart Object</h3>
                        <Button onClick={() => setIsCreating(true)} size="sm" className="bg-blue-500 text-white hover:bg-blue-600">
                    ...
                    {isLoading ? (
                        <Card>
                            <CardContent className="flex flex-col items-center justify-center py-8">
                                ...
                                <p className="text-muted-foreground mt-2">Caricamento policy...</p>
                            </CardContent>
                        </Card>
                    ) : (
                        smartObjects.map((smartObject) => (
                            <div key={smartObject.id} className="mb-6">
                                <h4 className="font-semibold mb-2">{formatName(smartObject.id)}</h4>
                                {(policiesBySmartObject[smartObject.id]?.length ?? 0) === 0 ? (
                                    <Card>
                                        <CardContent className="flex flex-col items-center justify-center py-8">
                                            <AlertTriangle className="h-12 w-12 text-muted-foreground mb-4" />
                                            <p className="text-muted-foreground">Nessuna policy configurata</p>
                                        </CardContent>
                                    </Card>
                                ) : (
                                                    ...
                                                    <div className="flex items-start justify-between">
                                                        <div className="flex flex-row gap-2 space-y-1 items-center">
                                                            ...
                                                            <div>
                                                                <CardTitle className="text-base">
                                                                    {(() => {
                                                                        const sensor = findSensorById(smartObject, policy.resource_id)
                                                                        const sensorType = sensor ? formatType(sensor.type) : policy.resource_id
                                                                        return SENSOR_TYPE_LABELS[sensorType as keyof typeof SENSOR_TYPE_LABELS] || sensorType
                                                                    })()}
                                                                </CardTitle>
                                                                <CardDescription>
                                                                    {policy.description}
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            <Button
                                                                variant="ghost"
                                                                size="sm"
                                                                onClick={() => handleEditPolicy(policy, smartObject)}
                                                            >
                                                            ...
                                                            <Button
                                                                variant="ghost"
                                                                size="sm"
                                                                onClick={() => handleDeletePolicy(policy.id, smartObject)}
                                                            >
                                                                ...
                                                                <div className="flex items-center gap-3">
                                                                    ...
                                                                    <div className="flex-1">
                                                                        <div className="text-sm font-semibold text-blue-900">
                                                                            {(() => {
                                                                                const sensor = findSensorById(smartObject, policy.resource_id)
                                                                                return sensor ? formatType(sensor.type) : "-"
                                                                            })()}
                                                                        </div>
                                                                        <div className="text-md font-bold text-blue-700">
                                                                            {getConditionText(policy.condition)} {findSensorById(smartObject, policy.resource_id)?.unit}
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                                <div className="space-y-2">
                                    ...
                                    <textarea
                                        ...
                                        onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) =>
                                            setEditingPolicy((prev) => prev ? { ...prev, policy: { ...prev.policy, description: e.target.value } } : null)
                                        }
                                ...
                                <div className="grid gap-4 md:grid-cols-2">
                                    <div className="space-y-2">
                                        ...
                                        <Select
                                            ...
                                            onValueChange={(value) => {
                                                setEditingPolicy((prev) => prev ? { ...prev, policy: { ...prev.policy, resource_id: value } } : null)
                                            }}
                                        >
                                            ...
                                            <SelectContent>
                                                {editingPolicy.smartObject.sensors?.map((sensor) => (
                                                    <SelectItem key={sensor.resource_id} value={sensor.resource_id}>
                                                        {sensor.resource_id}
                                                    </SelectItem>
                                                ))}
                                            </SelectContent>
                                                ...
                                                setEditingPolicy((prev) =>
                                                    prev
                                                        ? {
                                                            ...
                                                            policy: {
                                                                ...prev.policy,
                                                                condition: { ...prev.policy.condition, operator: value as any },
                                                            },
                                                        }
                                                        : null,
                                                )
                                            ...
                                            setEditingPolicy((prev) =>
                                                prev
                                                    ? {
                                                        ...
                                                        policy: {
                                                            ...prev.policy,
                                                            condition: { ...prev.policy.condition, value: Number.parseFloat(e.target.value) },
                                                        },
                                                    }
                                                    : null,
                                            )
                                ...
                                <div className="space-y-4">
                                    <Label className="text-base font-medium">Azione da Eseguire</Label>
                                    <div className="grid gap-4 md:grid-cols-2">
                                        <div className="space-y-2">
                                            <Label>Stato</Label>
                                            <Select
                                                value={editingPolicy.policy.action.command.value.status || ""}
                                                onValueChange={(value) =>
                                                    setEditingPolicy((prev) =>
                                                        prev
                                                            ? {
                                                                ...
                                                                policy: {
                                                                    ...
                                                                    action: {
                                                                        command: {
                                                                            value: { ...prev.policy.action.command.value, status: value as any },
                                                                        },
                                                                    },
                                                                },
                                                            }
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Input
                                                ...
                                                max="5"
                                                value={
                                                    editingPolicy.policy.action.command.value.speed || editingPolicy.policy.action.command.value.level || ""
                                                }
                                                onChange={(e) => {
                                ...
                                <div className="flex items-center justify-end pt-4">
                                    <div className="flex gap-2">
                                        ...
                                        <Button onClick={handleUpdatePolicy}>Salva Modifiche</Button>
                                    </div>
                                </div>
                            </CardContent>
                        </Card>
                    ...
                    {isCreating && (
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Configura una nuova policy automatica</CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-4">
                                <div className="space-y-2">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <textarea
                                        ...
                                        onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setNewPolicy((prev) => ({ ...prev, description: e.target.value }))}
                                        className="flex min-h-[80px] w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                                    />
                                </div>
                                <div className="grid gap-4 md:grid-cols-2">
                                    <div className="space-y-2">
                                        ...
                                        <Select
                                            ...
                                            onValueChange={(value) => {
                                                setNewPolicy((prev) => ({ ...prev, resource_id: value }))
                                            }}
                                        >
                                            <SelectTrigger className="w-full">
                                                <SelectValue placeholder="Seleziona sensore" />
                                            ...
                                            <SelectContent>
                                                {(smartObjects.find(so => so.id === newPolicySmartObjectId)?.sensors || []).map((sensor) => (
                                                    <SelectItem key={sensor.resource_id} value={sensor.resource_id}>
                                                        {sensor.resource_id}
                                                    </SelectItem>
                                                ))}
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Select
                                            ...
                                            onValueChange={(value) => updateNewPolicyCondition("operator", value)}
                                        >
                                            <SelectTrigger className="w-full">
                                                <SelectValue placeholder="Seleziona operatore" />
                                            </SelectTrigger>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => updateNewPolicyCondition("value", Number.parseFloat(e.target.value))}
                                    />
                                </div>
                                <Separator />
                                <div className="space-y-4">
                                    ...
                                    <div className="grid gap-4 md:grid-cols-2">
                                        <div className="space-y-2">
                                            ...
                                            <Select
                                                ...
                                                onValueChange={(value) => updateNewPolicyAction("status", value)}
                                            >
                                                <SelectTrigger className="w-full">
                                                    <SelectValue placeholder="Seleziona stato" />
                                                </SelectTrigger>
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Input
                                                ...
                                                min="0"
                                                max="5"
                                                value={newPolicy.action.command.value.speed || newPolicy.action.command.value.level || ""}
                                                onChange={(e) => {
                                                    const value = Number.parseInt(e.target.value)
