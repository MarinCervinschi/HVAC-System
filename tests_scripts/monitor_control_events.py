...
def subscribe_to_control_events(client, room_id="room_A1", rack_id="rack_A1"):
    ...
    # Subscribe to all relevant control event topics (rack-level and room-level devices)
    topics = [
        (
            f"{MqttConfigurationParameters.BASIC_TOPIC}/{room_id}/{MqttConfigurationParameters.RACK_TOPIC}/{rack_id}/{MqttConfigurationParameters.DEVICE_TOPIC}/+/{MqttConfigurationParameters.CONTROL_TOPIC}/+",
            1,
        ),
        (
            f"{MqttConfigurationParameters.BASIC_TOPIC}/{room_id}/{MqttConfigurationParameters.DEVICE_TOPIC}/+/{MqttConfigurationParameters.CONTROL_TOPIC}/+",
            1,
        ),
    ]
    ...
    if result == mqtt.MQTT_ERR_SUCCESS:
        print("📡 Subscribed to control events:")
